...
lst = ComponentDefinitionList(input_file)

#g.add((prov.Activity,RDF.about,myapp.biobrickassembly)) don't need both
bba = myapp['assembly/1']

Current attempt at adding retrospective provenance
g.add((bba,RDF.type,prov.Activity))
g.add((bba,prov.wasAssociatedWith,myapp['labs/ICOS']))
g.add((bba,dcterms.title,Literal("BioBrick Assembly Method")))
g.add((bba,myapp.assemblytype,myapp.BioBrick))
